              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full h-12 px-3 premium-input bg-black/40 text-sm focus:outline-none"
                  >
                    {LANGUAGES.map(l => (
              ...
              <AnimatePresence>
                {isScriptExpanded && (
                  <motion.div 
                    ...
                    animate={{ height: 'auto' }} 
                    exit={{ height: 0 }}
                    className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-transparent to-black/20 custom-scrollbar"
                  >
                    {script.length > 0 ? (
                      script.map((line, i) => (
                        <motion.div 
                          ...
                          className={`flex ${line.speaker === 'Host' || line.speaker === 'Speaker R' ? 'justify-start' : 'justify-end'}`}
                        >
                          <div className={`chat-bubble ${line.speaker === 'Host' || line.speaker === 'Speaker R' ? 'chat-bubble-user' : 'chat-bubble-ai'}`}>
